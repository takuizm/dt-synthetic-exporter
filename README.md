...
1. **ターミナル**で、プロジェクトフォルダに移動します：
   ```bash
   cd /Users/あなたのユーザー名/ダウンロード/dt-synthetic-exporter
   ```
...
3. **保存**: Command+S で保存し、テキストエディットを閉じる
...
| オプション | 説明 | 例 | 初心者おすすめ度 |
|-----------|------|-----|----------------|
| `--tag` | 特定タグのみ取得 | `--tag Owner:Koizumi` | ⭐⭐⭐ |
| `--hours` | 取得期間指定 | `--hours 48` | ⭐⭐⭐ |
| `--resolution` | データ間隔 | `--resolution 30m` | ⭐⭐ |
| `--output` | ファイル名指定 | `--output my_analysis` | ⭐⭐⭐ |
| `--no-geo` | 地域データ除外（高速化） | `--no-geo` | ⭐⭐ |
| `--monitor-analysis` | 監視別詳細分析 | `--monitor-analysis` | ⭐ |
